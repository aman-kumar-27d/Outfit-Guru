...
// Updated types for the new server response
type NewServerDetection = {
  source_model: string;
  label: string;
...
type NewServerResponse = {
  width: number;
  height: number;
  raw_detections: NewServerDetection[];
  filtered_detections: NewServerDetection[];
  refined_detections: NewServerDetection[];
  person_regions: Array<{
    person_bbox: [number, number, number, number];
    regions: {
      top: { bbox: [number, number, number, number]; dominant_color_rgb: [number, number, number]; dominant_color_hex: string };
      bottom: { bbox: [number, number, number, number]; dominant_color_rgb: [number, number, number]; dominant_color_hex: string };
      shoes: { bbox: [number, number, number, number]; dominant_color_rgb: [number, number, number]; dominant_color_hex: string };
    };
  }>;
};

// Legacy types for compatibility with analyze endpoint
type Detection = {
  label: string;
  confidence: number;
  bbox: [number, number, number, number];
...
export default function OutfitDetectorV2() {
  ...
  // Convert new server response to legacy format for compatibility
  const convertToLegacyFormat = (newResponse: NewServerResponse) => {
    // Filter refined detections (excluding 'person' detections for the main list)
    const filteredDetections = newResponse.refined_detections
      .filter(det => det.refined_label !== 'person')
      .map(det => ({
        label: det.refined_label,
        confidence: det.refined_confidence,
        bbox: det.bbox,
        dominant_color_hex: det.colors[0]?.hex || '#000000', // Use first color as dominant
        source_model: det.source_model
      }));

    // Convert person regions
    const convertedPersonRegions = newResponse.person_regions.map(person => ({
      person_bbox: person.person_bbox,
      regions: {
        top: {
          bbox: person.regions.top.bbox,
          dominant_color_hex: person.regions.top.dominant_color_hex
        },
        bottom: {
          bbox: person.regions.bottom.bbox,
          dominant_color_hex: person.regions.bottom.dominant_color_hex
        },
        shoes: {
          bbox: person.regions.shoes.bbox,
          dominant_color_hex: person.regions.shoes.dominant_color_hex
        }
      }
    }));
    ...
    return {
      ...
      personRegions: convertedPersonRegions
    };
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await fetch("http://localhost:8000/detect-v2", {
        method: "POST",
        body: formData,
      });
      ...
      <section className="min-h-screen bg-gray-50 flex items-center justify-center py-16">
        <div className="container mx-auto px-4">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-gray-900 mb-4">AI Outfit Detection V2</h2>
